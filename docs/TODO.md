...
**Status: done (working tree, uncommitted).** Unicode-range detection **and** a
verified legacy Kruti Dev signature detector: `detect.ts` derives the signature at
module load from `KRUTI_TO_UNICODE_TABLE` — every CP1252 extended-range byte (≥0x80)
in that verified table is an unambiguous Kruti Dev glyph code that can't occur in
plain Latin/English text, so one occurrence classifies as `legacy_krutidev`. No
character is added that isn't already in the sourced table (AI_RULES rule 10);
pure-ASCII Kruti Dev deliberately stays `unknown`. 9 tests in `detect.test.ts`
including real corpus files and rule-10 guards.
...
**Tests required:** unit tests covering clearly-Unicode input, clearly-Kruti Dev input, and clearly-neither input (must return `'unknown'`, not a guess) — plus the same golden-corpus samples run through detection to confirm they're classified correctly before conversion. **Done** — `detect.test.ts` (9 tests): Unicode positive, ASCII→unknown, empty→unknown, 4 real corpus files→`legacy_krutidev`, 2 pure-ASCII rule-10 guards.
